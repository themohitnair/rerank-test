...
async def main(k=100, score_threshold=0.3, rerank_top_percent=0.8):
    ...
    for model_key, model_info in MODELS.items():
        ...
        plot_reranker_performance_comparison(
            all_results, original_results, timing_data, model_info["name"]
        )

        print(f"\n📊 Generating topic distribution plots for {model_info['name']}...")
        plot_topic_distribution_comparison(
            all_results, original_results, model_info["name"]
        )
        ...
        if not summary_df.empty:
            ...
            best_improvement = summary_df.loc[
            ...
            if not fastest.empty:
                ...
                print(
                    f"   📈 Best Improvement: {best_improvement['Method']} (+{best_improvement['Precision_Improvement']:.3f})"
                )
                print(
                    f"   ⚡ Fastest: {fastest_reranker['Method']} ({fastest_reranker['Avg_Rerank_Time']:.3f}s)"
                )
